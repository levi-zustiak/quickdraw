...
export function useQuickdrawGame(opts: EngineOptions = {}): { state: GameState; actions: GameActions } {
  ...
  // Auto-join opponent after delay on invite screen
  useEffect(() => {
  ...
  return {
    state: {
      ...
      hudFlash, holsterArmed, toast, vsBot, spectators, muzzleFlash, startingHp,
    },
    actions: {
      createRoom, startVsBot, readyUp, armHolster, leaveHolster, fire, rematch, reset, setToast,
    },
  };
}
