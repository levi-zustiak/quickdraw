...
export type RoomEvent =
  ...
  | { type: 'ARM_HOLSTER'; socketId: string; stageWidth: number; stageHeight: number }
  | { type: 'LEAVE_HOLSTER'; socketId: string }
  | { type: 'CLIENT_SHOT'; socketId: string; reactionMs: number; dx: number; dy: number; targetSize: number }
  | { type: 'REQUEST_NEXT_ROUND' }
  | { type: 'REMATCH' }
  | { type: 'PLAYER_LEFT'; socketId: string }
  | { type: 'DEV_FORCE_START'; stageWidth: number; stageHeight: number };

function computeTargetSpawnCoords(minW: number, minH: number, targetSz: number) {
  const padX = targetSz / 2 + 12;
  const spawnMinY = 100 + targetSz / 2;
  const spawnMaxY = minH - 170 - targetSz / 2;
  const spawnX = (padX + Math.random() * Math.max(0, minW - padX * 2)) / minW;
  const spawnY = (spawnMinY + Math.random() * Math.max(0, spawnMaxY - spawnMinY)) / minH;
...
export const roomMachine = setup({
  ...
  guards: {
    ...
    anyHitRegistered: ({ context }) => {
      const p1Hit = context.p1Shot ? isHit(context.p1Shot.dist, context.p1Shot.targetSize) : false;
      const p2Hit = context.p2Shot ? isHit(context.p2Shot.dist, context.p2Shot.targetSize) : false;
      return p1Hit || p2Hit;
    },
  ...
  actions: {
    ...
    broadcastLobbyReady: ({ context, event }) => {
      const e = event as Extract<RoomEvent, { type: 'PLAYER_JOINED' }>;
      console.log(`[machine] lobby-ready  room=${context.roomCode} p1="${context.p1Name}" p2="${e.playerName}"`);
    ...
    removeP2AndReset: assign(({ context, event }) => {
      ...
      if (context.p1SocketId === e.socketId && context.p2SocketId) {
        return {
          ...
          p1Holstered: false, p2Holstered: false,
        };
      }
      return {
        p2SocketId: null as string | null,
    ...
    broadcastGameStart: ({ context }) => {
      console.log(`[machine] game-start  room=${context.roomCode} round=${context.round}`);
      context.io.to(context.roomCode).emit('game-start', { round: context.round });
    },
    ...
    applyArmHolster: assign(({ context, event }) => {
      ...
      console.log(`[machine] arm-holster  room=${context.roomCode} role=${role}`);
      context.io.to(context.roomCode).emit('player-holstered', { role });
      return isP1
        ? { p1Holstered: true, p1StageWidth: e.stageWidth, p1StageHeight: e.stageHeight }
        : { p2Holstered: true, p2StageWidth: e.stageWidth, p2StageHeight: e.stageHeight };
    }),
    ...
    beginArming: assign(({ context }) => {
      ...
      const minW = Math.min(context.p1StageWidth ?? 1280, context.p2StageWidth ?? 1280);
      const minH = Math.min(context.p1StageHeight ?? 720, context.p2StageHeight ?? 720);
      const targetSize = Math.max(120, Math.min(200, Math.min(minW, minH) * 0.35));
      const { spawnX, spawnY } = computeTargetSpawnCoords(minW, minH, targetSize);
      console.log(`[machine] both-holstered→arming  room=${context.roomCode} armingDelayMs=${armingDelayMs}`);
      context.io.to(context.roomCode).emit('target-spawn', {
        armingDelayMs, serverSpawnAt, spawnX, spawnY, targetSize, minStageWidth: minW, minStageHeight: minH,
      });
    ...
    devForceArm: assign(({ context, event }) => {
      ...
      return {
        p1Holstered: true, p2Holstered: true,
        p1StageWidth: e.stageWidth, p1StageHeight: e.stageHeight,
        p2StageWidth: e.stageWidth, p2StageHeight: e.stageHeight,
        armingDelayMs, targetSpawnedAt: serverSpawnAt,
      };
    }),
    ...
    broadcastFalseStart: assign(({ context, event }) => {
      const e = event as Extract<RoomEvent, { type: 'LEAVE_HOLSTER' }>;
      const isP1 = context.p1SocketId === e.socketId;
      console.log(`[machine] false-start  room=${context.roomCode} by=${isP1 ? 'p1' : 'p2'}`);
      context.io.to(context.roomCode).emit('false-start', { by: isP1 ? 'p1' as const : 'p2' as const });
      return {
        p1Holstered: false, p2Holstered: false,
        p1StageWidth: null as number | null, p1StageHeight: null as number | null,
        p2StageWidth: null as number | null, p2StageHeight: null as number | null,
      };
    }),
    ...
    recordShot: assign(({ context, event }) => {
      ...
      const shot: Shot = {
        reactionMs: e.reactionMs, dx: e.dx, dy: e.dy, dist,
        damage: computeDamage(dist, e.targetSize), targetSize: e.targetSize,
      };
      console.log(`[machine] client-shot  room=${context.roomCode} role=${isP1 ? 'p1' : 'p2'} reactionMs=${Math.round(e.reactionMs)}`);
    ...
    settleRound: assign(({ context }) => {
      ...
      const p1Hp = winner === 'p2' ? Math.max(0, context.p1Hp - damage) : context.p1Hp;
      const p2Hp = winner === 'p1' ? Math.max(0, context.p2Hp - damage) : context.p2Hp;
      console.log(`[machine] round-result  room=${context.roomCode} winner=${winner} damage=${Math.round(damage)} p1Hp=${p1Hp} p2Hp=${p2Hp}`);
      context.io.to(context.roomCode).emit('round-result', { winner, p1Shot, p2Shot, damage, p1Hp, p2Hp });
      return { p1Hp, p2Hp };
    }),

    advanceRound: assign(({ context }) => {
      const round = context.round + 1;
      console.log(`[machine] next-round  room=${context.roomCode} round=${round}`);
      context.io.to(context.roomCode).emit('next-round', { round });
      return { round, p1Shot: null, p2Shot: null, p1Holstered: false, p2Holstered: false };
    }),
    ...
    resetForRematch: assign(({ context }) => {
      ...
      return {
        p1Hp: STARTING_HP, p2Hp: STARTING_HP,
        p1Shot: null, p2Shot: null,
        p1Ready: false, p2Ready: false,
        p1Holstered: false, p2Holstered: false,
        round: 1,
      };
