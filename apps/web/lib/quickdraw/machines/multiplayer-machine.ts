...
export type MultiplayerEvent =
  ...
  | { type: 'GAME_SNAPSHOT'; phase: string; p1Name: string; p2Name: string; p1Hp: number; p2Hp: number; p1Ready: boolean; p2Ready: boolean; round: number; spectatorCount: number }
  | { type: 'SPECTATOR_COUNT'; count: number }
  | { type: 'JOIN_ERROR'; message: string }
  | { type: 'PLAYER_JOINED'; name: string; role: 'p1' | 'p2' }
  | { type: 'PLAYER_LEFT' }
  | { type: 'LOBBY_READY'; p1Name: string; p2Name: string | null; round: number }
  | { type: 'GAME_START'; round: number }
  | { type: 'TARGET_SPAWN'; armingDelayMs: number; serverSpawnAt: number; spawnX: number; spawnY: number; targetSize: number; minStageWidth: number; minStageHeight: number }
  | { type: 'DRAWING_START'; target: Target }
  | { type: 'ROUND_RESULT'; winner: 'p1' | 'p2' | null; p1Shot: Shot | null; p2Shot: Shot | null; damage: number; p1Hp: number; p2Hp: number }
  ...
  | { type: 'LEAVE_HOLSTER' }
  | { type: 'FIRE'; clientX: number; clientY: number; target: Target; spawnedAt: number }
  | { type: 'REMATCH' }
  | { type: 'RESET' }
...
export const multiplayerMachine = setup({
  ...
  actors: {
    spawnTimer: fromCallback<{ type: 'DRAWING_START'; target: Target }, SpawnPayload>(
      ({ input, sendBack }) => {
        const delay = Math.max(0, input.serverSpawnAt - (Date.now() + clockOffsetMs));
        const id = setTimeout(() => {
          const stage = document.querySelector('[data-qd-stage]');
          if (!stage) return;
          const rect = stage.getBoundingClientRect();
          const offsetX = (rect.width - input.minStageWidth) / 2;
          const offsetY = (rect.height - input.minStageHeight) / 2;
          const x = offsetX + input.spawnX * input.minStageWidth;
          const y = offsetY + input.spawnY * input.minStageHeight;
          const spawnedAt = performance.now();
          sendBack({ type: 'DRAWING_START', target: { x, y, size: input.targetSize, spawnedAt } });
        }, delay);
  ...
  actions: {
    setRoomAndConnect: assign(({ context, event }) => {
      if (event.type === 'CREATE_ROOM') {
        ...
        context.socket.emit('join-room', { roomCode: code, playerName: (event.name || 'YOU').toUpperCase() });
        return {
          roomCode: code,
          p1: makePlayer((event.name || 'YOU').toUpperCase(), context.startingHp),
      ...
      if (event.type === 'JOIN_ROOM') {
        ...
        if (!context.socket.connected) context.socket.connect();
        context.socket.emit('join-room', { roomCode: code, playerName: event.name.toUpperCase() });
      }
      return {};
    }),
    ...
    applyGameSnapshot: assign(({ context, event }) => {
      ...
      return {
        p1: { name: e.p1Name.toUpperCase(), hp: e.p1Hp, ready: e.p1Ready, wins: context.p1.wins },
        p2: { name: e.p2Name.toUpperCase(), hp: e.p2Hp, ready: e.p2Ready, wins: context.p2.wins },
        round: e.round,
    ...
    applyLobbyReady: assign(({ context, event }) => {
      ...
      return {
        p1: { ...context.p1, name: e.p1Name.toUpperCase(), hp: context.startingHp, ready: false },
        p2: { ...context.p2, name: (e.p2Name ?? 'OPPONENT').toUpperCase(), hp: context.startingHp, ready: false },
        round: e.round,
      };
    }),
    ...
    applyGameStart: assign(({ event }) => {
      ...
      return { round: e.round, shots: { p1: null, p2: null }, target: null, holsterArmed: false };
    }),
    ...
    armHolsterAndEmit: assign(({ context, event }) => {
      ...
      context.socket.emit('arm-holster', { roomCode: context.roomCode, stageWidth: e.stageWidth, stageHeight: e.stageHeight });
    ...
    applyFalseStart: assign(({ context, event }) => {
      ...
      const msg = e.by === context.myRole
        ? 'False start! Re-holster when ready…'
        : 'Opponent flinched! Holster up again…';
      return { target: null, holsterArmed: false, toast: msg, spawnPayload: null };
    }),

    applyHolsteredToast: assign(({ context, event }) => {
      const e = event as Extract<MultiplayerEvent, { type: 'PLAYER_HOLSTERED' }>;
      if (e.role !== context.myRole) return { toast: 'Opponent in holster — step up!' };
      return {};
    }),
    ...
    fireAndEmit: assign(({ context, event }) => {
      ...
      const dx = e.clientX - e.target.x;
      const dy = e.clientY - e.target.y;
      context.socket.emit('client-shot', { roomCode: context.roomCode, reactionMs, dx, dy, targetSize: e.target.size });
    ...
    applyRoundResult: assign(({ context, event }) => {
      const e = event as Extract<MultiplayerEvent, { type: 'ROUND_RESULT' }>;
      return {
        p1: { ...context.p1, hp: e.p1Hp, wins: e.winner === 'p1' ? context.p1.wins + 1 : context.p1.wins },
        p2: { ...context.p2, hp: e.p2Hp, wins: e.winner === 'p2' ? context.p2.wins + 1 : context.p2.wins },
        shots: { p1: e.p1Shot, p2: e.p2Shot },
        hudFlash: { p1: e.winner === 'p2', p2: e.winner === 'p1' },
        muzzleFlash: false,
        history: [...context.history, { round: context.round, winner: e.winner, p1Shot: e.p1Shot, p2Shot: e.p2Shot, damage: e.damage }],
      };
    }),
  ...
  on: {
    ...
    SPECTATOR_COUNT: { actions: assign(({ event }) => ({ spectators: (event as Extract<MultiplayerEvent, { type: 'SPECTATOR_COUNT' }>).count })) },
  ...
  states: {
    ...
    connecting: {
      on: {
        JOINED: [
          {
            guard: ({ event }) => (event as Extract<MultiplayerEvent, { type: 'JOINED' }>).role === 'spectator',
            target: 'spectating',
            actions: 'applyJoined',
          },
          {
        ...
        JOIN_ERROR: {
          target: 'landing',
          actions: assign(({ event }) => ({ toast: (event as Extract<MultiplayerEvent, { type: 'JOIN_ERROR' }>).message })),
        },
      },
    },
    ...
    lobby: {
      on: {
        ...
        PLAYER_LEFT: { target: 'invite', actions: assign({ toast: 'Opponent disconnected.' }) },
        RESET: { target: 'landing', actions: 'resetAndDisconnect' },
      },
    },
      ...
      states: {
        ...
        drawing: {
          on: {
            ...
            ROUND_RESULT: [
              { guard: 'gameOver', target: '#multiplayer.gameover', actions: 'applyRoundResult' },
